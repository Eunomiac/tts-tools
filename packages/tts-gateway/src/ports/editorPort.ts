...
/** Never kill the IDE that owns/spawns the gateway. */
const isIdeProcess = (listener: EditorPortListener): boolean => {
  const name = listener.name.trim().toLowerCase();
  return (
    name === "cursor" ||
    name === "code" ||
    name === "code - insiders" ||
    name.includes("code - insiders") ||
