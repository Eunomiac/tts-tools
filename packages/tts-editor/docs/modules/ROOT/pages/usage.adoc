...
Use **Release TTS Editor Port** (status bar or Command Palette) to free the port for the Storyteller Dashboard without restarting Cursor.
Use **Claim TTS Editor Port** to take it back (on Windows this force-stops other reclaimable listeners; it never kills Tabletop Simulator).
