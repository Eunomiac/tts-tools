...
| Get Object
| Refresh **one** selected object from the **live** table in TTS (`getObjectFromGUID` + `getJSON`).
Rewrites that object's `.tts` data/script/UI files without touching the rest of the TTS Objects list.
Does **not** read from a save file on disk — only the running game.
